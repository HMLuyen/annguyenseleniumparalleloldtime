...
public class ReporterManager implements IReporter{
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
        ...
        generateSimpleMethodReport(suites);
...
//        generateMethodDetailReport(suites);
    ...
    /**
     * Creates a table showing the highlights of each test method without links to
     * the method details and pass/fail sorter
     */

    protected void generateSimpleMethodReport(List<ISuite> suites) {
        m_methodIndex = 0;
        startSimpleRultSumaryTable("methodOverview");
        int testIndex = 1;
        for (ISuite suite : suites) {
            if (suites.size() >= 1) {
                titleRow(suite.getName(), 5);
            ...
            for (ISuiteResult r2 : r.values()) {
                ...
                String testName = testContext.getName();
                m_testIndex = testIndex;
                resultSimpleSummary(suite, testContext.getFailedConfigurations(), testName, "failed", " (configuration methods)");
                resultSimpleSummary(suite, testContext.getFailedTests(), testName, "failed", "");
                resultSimpleSummary(suite, testContext.getSkippedConfigurations(), testName, "skipped", " (configuration methods)");
                resultSimpleSummary(suite, testContext.getSkippedTests(), testName, "skipped", "");
                resultSimpleSummary(suite, testContext.getPassedTests(), testName, "passed", "");
                testIndex++;
            }
        }
        writer.println("</table>");
     ...
     * @param tests
     */
    private void resultSimpleSummary(ISuite suite, IResultMap tests, String testname,
    ...
    /** Starts and defines columns result summary table for generateMethodDetailReport methods */
    ...
    /** Starts and defines columns result summary table for generateSimpleMethodReport methods */
    private void  startSimpleRultSumaryTable(String style) {
        tableStart(style, "summary");
        writer.println("<tr><th>Class</th>"
                + "<th>Method</th><th>Result</th><th>Exception Info</th><th>Execution Time<br/>(hh:mm:ss)</th></tr>");
        m_row = 0;
    }
    ...
    private void resultSimpleDetail(IResultMap tests) {
        Set<ITestResult> testResults=tests.getAllResults();
        List<ITestResult> testResultsList = new ArrayList<ITestResult>(testResults);
        Collections.sort(testResultsList, new TestResultsSorter());
        for (ITestResult result : testResultsList) {
